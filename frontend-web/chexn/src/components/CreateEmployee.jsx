...
function CreateEmployee({ onCreated }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setMessage('Creating employee account...');

    try {
      await apiClient.post('/employer-staff/create-employee', {
        email,
        password,
        firstName,
        lastName
      ...
      <form onSubmit={handleSubmit} className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="md:col-span-2">
          <button type="submit" disabled={isSubmitting} className="w-full rounded-md bg-blue-600 hover:bg-blue-700 text-white px-4 py-3 font-medium disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2">
            {isSubmitting ? (<><Spinner /> <span className="text-white text-sm">Creating...</span></>) : 'Create Employee'}
          </button>
        </div>
      </form>
