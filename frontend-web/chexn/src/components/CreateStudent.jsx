...
function CreateStudent({ onCreated }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await apiClient.post('/staff/create-student', {
        email,
        password,
        firstName,
        lastName
    ...
    <div className="space-y-4">
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-4 md:p-5">
        <div className="flex items-center justify-between gap-2 mb-4">
          <h3 className="text-lg font-semibold text-gray-900">Create Student Account</h3>
          <InfoTooltip description="Generate a student login so they can check in and stay connected to your support team." />
        </div>
        ...
        <div className="mb-4">
          ...
          >
            {showImport ? '← Single Entry' : '📥 Bulk Import Students'}
          </button>
        </div>
        
        {showImport ? (
          <BulkImport
            endpoint="/staff/bulk-create-students"
      ...
      <form onSubmit={handleSubmit} className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="md:col-span-2">
          <button type="submit" disabled={isSubmitting} className="w-full rounded-md bg-blue-600 hover:bg-blue-700 text-white px-4 py-3 font-medium disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2">
            {isSubmitting ? (<><Spinner /> <span className="text-white text-sm">Creating...</span></>) : 'Create Student'}
          </button>
        </div>
      </form>
      {message && (
